...
export const typeProxyHandler = new Proxy(Logger, {
    get(target, prop, receiver) {
        ...
        if (prop in target) {
            ...
            if (['plain', 'info', 'warn', 'error', 'debug', 'success', 'failure'].includes(prop)) {
                return value;
            }
            return value;
        }
        // For custom types not in target, return a function that can create and register the custom type
        const customTypeFunction = function (...args) {
            ...
            if (styles && Array.isArray(styles)) {
                ...
                throw new TypeError(`Invalid arguments for adding a new logger getter.`);
            }
        };
        ...
        return new Proxy(customTypeFunction, {
            get(fnTarget, fnProp, fnReceiver) {
                ...
                try {
                    ...
                    const customTypes = loggerInstance.constructor.registeredTypes || [];
                    if (customTypes.includes(prop)) {
                        const logger = Logger.getLoggerInstance(prop);
                        if (fnProp in logger) {
        ...
        try {
            ...
            if (customTypes.includes(prop)) {
                return {
                    enumerable: true,
                    configurable: true,
                    value: Logger.getLoggerInstance(prop),
                };
            }
        }
        catch {
