...
export class BrowserLogger extends BaseLogger {
    ...
    static type(type, styles) {
        if (type in BrowserLogger && styles) {
            safeConsoleLog(`%cLogger type "${String(type)}" is preset. Add custom getter will override the preset.`, 'color: #ffff00; text-decoration: underline');
        }
        if (styles) {
 ...
 */
export class BrowserStreamLogger extends BaseLogger {
    _state = undefined;
    _delay = 0;
